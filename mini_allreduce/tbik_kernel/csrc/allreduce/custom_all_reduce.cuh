...
DINLINE P packed_tree_reduce(const P* ptrs[], int idx) {
  static_assert(ngpus == 1 || ngpus == 2 || ngpus == 4 || ngpus == 8,
                "custom_tree_all_reduce only supports ngpus in (1,2,4,8)");
  if constexpr (ngpus == 1) {
    return ptrs[0][idx];
  } else if constexpr (ngpus == 2) {
    P sum01 = ptrs[0][idx];
...
template <typename T, int ngpus>
__global__ void __launch_bounds__(512, 1) custom_tree_all_reduce(
    RankData* _dp, RankSignals sg, Signal* self_sg, T* __restrict__ result, int rank, int size) {
  using P = typename packed_t<T>::P;
  using A = typename packed_t<T>::A;
  auto dp = *_dp;
  multi_gpu_barrier<ngpus, true>(sg, self_sg, rank);
  for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < size; idx += gridDim.x * blockDim.x) {
    ((P*)result)[idx] = packed_tree_reduce<P, ngpus, A>((const P**)&dp.ptrs[0], idx);
  }
  multi_gpu_barrier<ngpus, false>(sg, self_sg, rank);
}
...
#undef KL
  ...
  template <typename T>
  void tree_allreduce(
      cudaStream_t stream, T* input, T* output, int size, int threads = 512, int block_limit = 36) {
    auto d = packed_t<T>::P::size;
    if (size % d != 0)
      throw std::runtime_error(
...
#define KL(ngpus, name) name<T, ngpus><<<blocks, threads, 0, stream>>>(ptrs, sg_, self_sg_, output, rank_, size);
    switch (world_size_) {
      case 1:
        KL(1, custom_tree_all_reduce);
        break;
      case 2:
        KL(2, custom_tree_all_reduce);
        break;
      case 4:
        KL(4, custom_tree_all_reduce);
        break;
      case 8:
        KL(8, custom_tree_all_reduce);
        break;
      default:
        throw std::runtime_error(
            "custom tree allreduce only supports num gpus in (1,2,4,8). Actual num "
            "gpus = " +
            std::to_string(world_size_));
    }
#undef KL
