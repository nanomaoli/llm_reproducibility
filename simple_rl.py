...
def main():
    ...
    # Training config
    group_size = 2  # Samples per prompt for GRPO (increased from 4)
    ...
    use_vllm_compat = batch_invariant_is_enabled() or tp_invariant_is_enabled() or compatible_mode_is_enabled()

    if use_vllm_compat:
        print("✓ Patches detected - using vLLM-compatible model in torchtitan")
    else:
        print("⚠ Patches NOT detected - using standard model in torchtitan")
