...
from .patch_training_inference_align import patch_training_inference_align as patch_align, patch_training_baseline
...
def apply_patches():
    ...
    if not batch_invariant_is_enabled() and not tp_invariant_is_enabled() and not compatible_mode_is_enabled():
        patch_training_baseline()
        print("No patches applied to vLLM. Only applying baseline patches (UnifiedTritonAttention)\n")
        return
    if batch_invariant_is_enabled():
        patch_batch_invariant()
    if tp_invariant_is_enabled():
        patch_tp_invariant()
