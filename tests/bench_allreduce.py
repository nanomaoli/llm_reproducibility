...
def _benchmark_custom_tree_ar(inp: torch.Tensor, custom_tree_ar, iters: int) -> float:
    latencies = []
    for _ in range(iters):
        buf = inp.clone()
        torch.cuda.synchronize()
        start = time.perf_counter()
        out = custom_tree_ar.custom_tree_all_reduce(buf)
        torch.cuda.synchronize()
        if out is None:
            return float("nan")
        latencies.append(time.perf_counter() - start)
...
def _worker(rank: int, world_size: int, port: int, args):
    ...
    from mini_allreduce import CustomAllreduce, CustomTreeAllreduce
    ...
    if rank == 0:
        ...
        if custom_tree_ar.disabled:
            print("Custom tree all-reduce disabled.")
        else:
            print("Custom tree all-reduce enabled.")
        if not tree_enabled:
    ...
    for numel in args.numel_list:
        ...
        lat_custom_tree = float("nan")
        if not custom_tree_ar.disabled:
            lat_custom_tree = _benchmark_custom_tree_ar(inp, custom_tree_ar, args.iters)
        ...
        if rank == 0:
            print(
                ...
                f"custom={lat_custom*1e3:>8.3f} ms "
                f"custom_tree={lat_custom_tree*1e3:>8.3f} ms "
                f"tree={lat_tree*1e3:>8.3f} ms"
            )
